...
4. 按住卡片左下角的六点手柄拖动，即可调整当前位置；搜索时需先清空关键词。
5. 聚焦拖动手柄后，也可以按 `Alt + ↑/↓` 移动卡片。
6. 点击左上角返回箭头回到全部分类。
7. 点击面板右上角箭头收起；点击右侧紫色标签再次展开。

## 升级说明

数据模型会自动兼容旧版本：1.0 的平铺内容会迁入“未分类”，1.1 的两级数据会按升级前的显示顺序生成排序编号。正文格式、层级关系、创建时间和更新时间都会保留。
...
`npm run check` 会校验 Manifest 引用和 JavaScript 语法。`npm test` 包含 9 项模型测试，覆盖旧数据迁移、层级关系、跨层搜索、内容格式保留和持久化排序；此外项目已通过真实 Edge 拖放与层级流程验证。
